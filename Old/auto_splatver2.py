...
#============================= Inputs =========================================
Txinput = "TxTest.csv"                                                          #CHANGE TRANSMITTER FILE NAME
Rxinput = "RxTest.csv"                                                          #CHANGE RECEIVER FILE NAME
Receiverheight = "10m"                                                          # Change Receiver height
Transmitterheight = "150m"
numtrans = 6
numrec = 6
Outfilename = "New2.0out.csv"

#======================== Read in Files ======================================

house_df = pd.read_csv(Txinput,names =['Farm', 'Lat', 'Long'], delim_whitespace=True)                  
...
def create_qth_file():
  for i in range(len(house_df['Farm'])): 
    ...
    Height = Transmitterheight                                                                                 #!!!!!!!!!!!!!!CHANGE TRANSMITTER HEIGHT
...
def create_qth_file_dish():
  ...
  return
create_qth_file_dish()   

#=========== Creates Pana Dataframe with headings ============================

Tx_cols = ['Transmitter', 'Tx location', 'Tx elevation', 'Tx height',
'Tx height above avg terrain', 'Distance to Rx','Azimuth to Rx', 
'Depression angle to Rx', 'Dep angle to first obstruction from Tx']
Tx_df = pd.DataFrame(columns=Tx_cols)

Rx_cols = ['Receiver', 'Rx location', 'Rx Elevation', 'Rx height',
' Rx height above avg terrain', 'Distance to Tx', 'Azimuth to Tx', 
'Depression angle to Tx','Dep angle to first obstruction from Rx']
Rx_df = pd.DataFrame(columns=Rx_cols)

ITM_cols = ['Earth Dielectric Constant', 'Earth Conductivity', 
'Atmospheric Bending Constant', 'Frequency', 'Radio Climate', 'Polarization',
'Fraction of Situations', 'Fraction of Time', 'Transmitter ERP', 'Transmitter EIRP']
ITM_df = pd.DataFrame(columns=ITM_cols) 

Summary_cols = ['FSPL', 'ITWOM Version3 PL','Attenuation due to terrain', 
'Field strength at Rx', 'Signal power level at Rx', 'Signal power density at Rx',
'Voltage across 50ohm dipole at Rx', 'Voltage across 75ohm dipole at Rx', 'Mode of propagation']
