...
house_df = pd.read_csv('TxTest.csv',names =['Farm', 'Lat', 'Long'], delim_whitespace=True)   #!!!!!!!!!!!!!!CHANGE TRANSMITTER FILE NAME
dish_df = pd.read_csv('RxTest.csv',names =['Dish', 'D_Long', 'D_Lat'], delim_whitespace=True)    #!!!!!!!!!!!!!!CHANGE RECEIVER FILE NAME
...
def create_qth_file():
  ...
  for i in range(len(house_df['Farm'])): 
    ...
    Height = '150m'                                                                                    #!!!!!!!!!!!!!!CHANGE TRANSMITTER HEIGHT
...
def create_qth_file_dish():
  ...
  for i in range(len(dish_df['Dish'])): 
    ...
    Dish_Height = '10m'                                                                                 #!!!!!!!!!!!!!!CHANGE RECEIVER HEIGHT
...
Summary_cols = ['FSPL', 'ITWOM Version3 PL','Attenuation due to terrain', 'Field strength at Rx', 'Signal power level at Rx', 'Signal power density at Rx',
...
for i in range(no_of_dishes):
  for j in range(no_of_houses):
   # print "###################################",str(house_df['Farm'][j]),"################################"
    ...
    os.system("splat -t "+str(house_df['Farm'][j])+" -r "+str(dish_df['Dish'][i])+" -metric")
    ...
    info = [word.strip() for word in (contents.split('ITWOM error number:')[0].split('---------------------------------------------------------------------------'))]
    ...
    Results_df.to_csv('TestingOut.csv')                                                                #!!!!!!!!!!!!!!CHANGE OUTPUT FILE NAME
